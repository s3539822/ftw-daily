...
const FieldCurrencyInputComponent = props => {
  const { rootClassName, className, id, label, input, meta, labelId, onChange, ...rest } = props;
  ...
  const inputProps = { className: inputClasses, id, input, onChange, ...rest };
  const classes = classNames(rootClassName, className);
  return (
    <div className={classes}>
      {label ? <label htmlFor={id} id={labelId}>{label}</label> : null}
